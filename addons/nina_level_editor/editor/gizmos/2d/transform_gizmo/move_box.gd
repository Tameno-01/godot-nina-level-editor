...
const HOVER_LENIENCE: float = 1.5
const COLOR: Color = NinaEditor.COLOR_XY
...
func _get_hover_priority(new_position: Vector2) -> float:
	...
	var distance: float = max(abs(texture_relative_pos.x), abs(texture_relative_pos.y))
...
func _on_click() -> void:
	get_parent().component_clicked(NinaTransformGizmo2D.ComponentTypes.XY_MOVE)
